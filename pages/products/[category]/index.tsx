...
export default function ProductCategoryPage({ category, modules }: CategoryPageProps) {
  const formattedCategory = category.charAt(0).toUpperCase() + category.slice(1);
  
  return (
    <>
      <Head>
        <title>{formattedCategory} | rolodexterLABS</title>
        <meta name="description" content={`rolodexterLABS knowledge on ${category}`} />
      </Head>
      
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="mb-12">
          <h1 className="text-4xl font-bold mb-2">{formattedCategory}</h1>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {modules.map((module) => (
            <Link 
              href={`/products/${category}/${module.slug}`} 
              key={module.slug}
  ...
  const categories = Array.from(
    new Set(
      modules
        ...
        .filter(category => category.startsWith('products/'))
