...
export default function CategoryPage({ category, modules }: CategoryPageProps) {
  const formattedCategory = category.charAt(0).toUpperCase() + category.slice(1);
  
  return (
    <>
      <Head>
        <title>{formattedCategory} Knowledge | rolodexterLABS</title>
        <meta name="description" content={`rolodexterLABS knowledge modules on ${category}`} />
      </Head>
      
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="mb-12">
          <h1 className="text-4xl font-bold mb-2">{formattedCategory} Knowledge</h1>
          <p className="text-xl text-gray-600">
            Curated knowledge modules on {category} topics
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {modules.map((module) => (
            <Link 
              href={`/research/${category}/${module.slug}`} 
              key={module.slug}
...
export const getStaticProps: GetStaticProps<CategoryPageProps, Params> = async ({ params }) => {
  ...
  const categoryPath = `research/${category}`;
  ...
  const categories = Array.from(
    new Set(
      modules
        ...
        .filter(category => category.startsWith('research/'))
