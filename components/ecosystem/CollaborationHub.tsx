...
const CollaborationHub: React.FC<CollaborationHubProps> = ({ activities, agents }) => {
  ...
  useEffect(() => {
    ...
    const newNodes: Node[] = [
      ...
      ...activities.slice(0, 50).map(activity => ({
        id: activity.id,
        label: activity.content.length > 30 
    ...
    const newLinks: Link[] = [
      ...
      ...activities.slice(0, 50).flatMap(activity => 
        ...
        newNodes.some(n => n.id === link.source) && 
        newNodes.some(n => n.id === link.target)
      )
    ];
    ...
    const animate = () => {
      ...
      nodes.forEach(node => {
        ...
        if (node === hoveredNode || node.type === 'agent') {
          ...
          ctx.fillText(
            node.type === 'agent' ? node.label : node.label.substring(0, 20) + (node.label.length > 20 ? '...' : ''),
            node.x,
  ...
  function getActivityColor(type: string, opacity = 1) {
    switch(type) {
      case 'research': return `rgba(64, 151, 242, ${opacity})`;
      case 'analysis': return `rgba(242, 174, 64, ${opacity})`;
      case 'connection': return `rgba(106, 212, 134, ${opacity})`;
      case 'insight': return `rgba(242, 64, 122, ${opacity})`;
      case 'question': return `rgba(165, 94, 234, ${opacity})`;
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div 
        ref={containerRef} 
        className="relative flex-grow overflow-hidden bg-lab-gray-900"
      >
        <canvas 
          ref={canvasRef}
          style={{
